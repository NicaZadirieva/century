...
function reducer(state, action) {
    ...
    switch(action.type) {
        ...
        case "CHANGING_FIELD":
            {
             ...
             if(state.current_telephone !== phone) window.localStorage.setItem("telephone", phone);
             if(state.current_region !== region) window.localStorage.setItem("region", region);
             if(state.current_message !== message) window.localStorage.setItem("message", message); 
             return {...state, ...{current_fio:fio, current_telephone: phone, current_region: region, current_message: message}};
            }
        case "SHOWING_FORM":
            {
        ...
        case "SUCCESS":
            ...
            return {...state,...{current_fio, current_telephone, current_region, current_message}};
            }
        case "FAILURE":
        {    document.querySelector(".cssload-thecube").style.display = "none";
            document.querySelector(".btn-contact").disabled = false;
